...
#'   MNO.
#'   
#' @param filter boolean. If TRUE removes from the dataset stationary 'duplicit' devices (i.e. two or more devices which stay connected for the 
#'  whole time span to the same antenna.). Implicit value is FALSE.
#'
...
#'
#' @export
readEvents <- function(eventsFileName, simulatedData = TRUE, filter = FALSE) {
  ...
  if (simulatedData == TRUE) {
    ...
    if(filter == TRUE) {
      ...
      p <- events.dt[, .(time, antennaID), by = .(deviceID)]
      p <- p[,n := uniqueN(antennaID), by = .(deviceID)]
      p <- p[n == 1,]
      p <- unique(p[,.(deviceID, antennaID, n)])
      p <- p[duplicated(antennaID,n) | duplicated(antennaID, n, fromLast = TRUE),]
      events.dt <- events.dt[!(deviceID %in% p$deviceID),]
    }
